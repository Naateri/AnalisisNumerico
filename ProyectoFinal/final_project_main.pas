...
uses
  Classes, SysUtils, FileUtil, uCmdBox, TAGraph, TAFuncSeries, TASeries,
  TATools, Forms, math, Controls, Graphics, Dialogs, Grids, StdCtrls, ExtCtrls,
  nle_methods, lagrange_pol, Integrals, edo, MyMatrix, NonLinearSystem, TAChartUtils,
  ParseMath, Types;
...
var
  i: Integer;
begin
  functions_list := TStringList.Create;
  clickFunc1:= False;
  error := 0.01;
  decimal := 2;
...
//            root_finder.b:= StrToFloat(my_list[2]);
            ...
            if (root_finder.found_solution = False) then begin
                ShowMessage('No se encontro una raiz.');
                memHistorial.Text:= memHistorial.Text + 'Root(s) not found.' + LineEnding;
            end else begin
            ...
            Chart1.Visible:= True;
            Result := res;
            functions_list.Add(root_finder.func);
            //root_finder.Destroy;
...
begin
     lagrange_solver := TLagrange.create;
            ...
            if (lagrange_solver.num_points > 6) then
               ShowMessage('Recomendacion: Solo tomar 6 puntos.');
...
begin
     integral_solver := TIntegralMethods.create;
              ...
              functions_list.Add(integral_solver.func);
              le_parser.Expression:= my_list[1];
...
//            integral_solver.b := StrToFloat(my_list[3]);
            ...
            le_area.UseZeroLevel:= True;
            le_area.AreaBrush.Color:= clWhite;
            le_area.AreaContourPen.Color:= clRed;
            le_area.AreaContourPen.Style:= psDot;
            le_area.AreaContourPen.Width:= 3;
            le_area.AreaLinesPen.Style:= psClear;
            ...
            functions_list.Add(current_func);
...
{            le_area_inter.Active:= True; }
            ...
            functions_list.Add(current_func2);
...
//            ShowMessage('Grafica. (f,a,b,color)'); SYNTAX
          ...
          'senl': begin
            ...
            for i := iPos + 1 to 3 do //if less than 4 equations
              ...
              array_list.Clear;
            SetLength(senl_solver.variables, 4);
            temp := Trim(my_list[0]);
            iPos := Pos( ']', temp);
            array_list.Delimiter:= ',';
            array_list.StrictDelimiter:= True;
            ...
            for iPos := 0 to array_list.Count - 1 do begin
              senl_solver.variables[iPos] := find_string(Copy(array_list[iPos],2,Length(array_list[iPos])-2));
            end;

            {for i := iPos + 1 to 3 do //if less than 4 equations
                senl_solver.variables[i] := 'abcd'; }
         ...
         + 'update(var_name; var_value): actualiza el valor de una variable' + LineEnding
          ...
          + 'root(f;a;b;true/false;method): f es la función, a y b el intervalo, los métodos pueden ser: ' +
           'Biseccion (0), Falsa Posicion (1), Secante (2). True (1) o False (0)' +
            ...
            'method: 0->Euler, 1->Heun, 2->Runge Kutta cuarto orden, 3->Dormand-Prince, 4->rk-fehlberg. ' + LineEnding
            + 'sedo([differential_functions];x0;[vars_initial_values];xn): utilizando Runge Kutta de cuarto orden, '
            + 'resuelve el sistema de ecuaciones diferenciales ordinarias. (Maximo 2, variables y z). ' + LineEnding
            );
...
//       ShowMessage('x: ' + FloatToStr(x) + ', y: ' + FloatToStr(y));
       if (clickFunc1 = False) then begin
          for i := 0 to functions_list.Count - 1 do begin
            le_parser.Expression:= functions_list[i];
            le_parser.NewValue('x', x);
            temp:= le_parser.Evaluate();
            if ( abs(temp - y) <= 0.1) then begin
               func1:= functions_list[i];
               ShowMessage('funcion seleccionada: ' + func1);
               clickFunc1 := True;
               Break;
            end;
          end;
       ...
       if (clickFunc1 = True) then begin
          //newA := (pg.X + tempX) / 2;
          methods.a:= Chart1.Extent.XMin;
          methods.func:= func1;
          methods.b:= Chart1.Extent.XMax;
          for i := 0 to functions_list.Count - 1 do begin
//            ShowMessage('current func: ' + functions_list[i]);
            le_parser.Expression:= functions_list[i];
            le_parser.NewValue('x', x);
            temp:= le_parser.Evaluate();
            if ( abs(temp - y) <= 0.1) then begin
               Methods.func2 := functions_list[i];
               ShowMessage('funcion seleccionada: ' + Methods.func2);
               Break;
            end;
          end;

          if (Methods.func = Methods.func2) then begin
             ShowMessage('Ha seleccionado la misma funcion dos veces. Vuelva a intentarlo.');
             clickFunc1:= False;
             Exit;
          end;
          Methods.Parse := le_parser;
          Res:= Methods.Intersection();
          LineSeries := TLineSeries.Create(Chart1);
          LineSeries.ShowLines:= False;
          LineSeries.ShowPoints:= True;
          for i := 0 to Methods.results_size - 1 do begin
              LineSeries.AddXY(Methods.results[i], Methods.f2(Methods.results[i]));
          end;
          LineSeries.ShowPoints:= True;
          LineSeries.ShowLines:= False;

          Chart1.AddSeries( LineSeries );
          memHistorial.Text:= memHistorial.Text + 'Intersection' + LineEnding;
          memHistorial.Text:= memHistorial.Text + 'Raices: ';
          for i := 0 to Methods.results_size - 1 do begin
              memHistorial.Text := memHistorial.Text + '(' + FloatToStr(Methods.results[i]) + ', ' +
              FloatToStr( Methods.f2(res)) + ')' + ', ';
          end;
          memHistorial.Text := memHistorial.Text + LineEnding;
          clickFunc1 := False;
